...
export async function GET(req) {
    ...
    try {
        const res = await fetch(
            `${API_BASE}/countries/${encodeURIComponent(country.trim().toUpperCase())}/states`,
            {
        ...
        if (!res.ok) {
            ...
            return NextResponse.json(
                { success: false, error: err.error || "Failed to fetch states" },
                { status: res.status }
            );
        }
        ...
        console.error("Geo states API error:", error);
        return NextResponse.json(
