...
export default function TicketsPage() {
    ...
    const fetchUsers = async () => {
        try {
            ...
            if (data.success)
                setUsers(data.data.filter((u) => ["admin", "manager"].includes(u.role)));
        } catch (error) {
        ...
        <ContentWrapper>
            ...
            {isStaff && (
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <Card
                        className="p-4 cursor-pointer hover:shadow-md transition-shadow"
                        onClick={() => setFilterStatus("waiting_staff")}
                    >
                        <div className="flex items-center gap-3">
                ...
                {loading ? (
                    ...
                    filteredTickets.map((ticket) => (
                        <Card
                            ...
                            style={{
                                borderLeftColor:
                                    priorityColors[ticket.priority]?.dot.replace("bg-", "") ===
                                    "gray-400"
                                        ? "#9ca3af"
                                        : priorityColors[ticket.priority]?.text.includes("red")
                                          ? "#dc2626"
                                          : priorityColors[ticket.priority]?.text.includes("orange")
                                            ? "#ea580c"
                                            : priorityColors[ticket.priority]?.text.includes("blue")
                                              ? "#2563eb"
                                              : "#9ca3af",
                            }}
                            ...
                            <div className="flex items-start justify-between gap-4">
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <Badge
                                            variant={statusConfig[ticket.status]?.color}
                                            size="sm"
                                        ...
                                        <span
                                            className={`text-xs px-2 py-0.5 rounded-full ${priorityColors[ticket.priority]?.bg} ${priorityColors[ticket.priority]?.text}`}
                                        >
                                            {ticket.priority}
                                        </span>
                                    </div>
                    ...
                    <div className="flex flex-col h-[70vh]">
                        ...
                        <div className="border-b dark:border-gray-700 pb-4 mb-4">
                            ...
                            <div className="flex flex-wrap items-center gap-2 mt-2">
                                <Badge variant={statusConfig[selectedTicket.status]?.color}>
                                    {statusConfig[selectedTicket.status]?.label}
                                </Badge>
                                <span
                                    className={`text-xs px-2 py-1 rounded-full ${priorityColors[selectedTicket.priority]?.bg} ${priorityColors[selectedTicket.priority]?.text}`}
                                >
                            ...
                            {selectedTicket.messages?.map((msg, idx) => (
                                ...
                                >
                                    ...
                                    >
                                        <div className="flex items-center gap-2 mb-1">
                                            <span
                                                className={`text-xs font-semibold ${msg.sender?._id === user?._id && !msg.isInternal ? "text-purple-200" : "text-gray-500"}`}
                                            >
                                            ...
                                            <span
                                                className={`text-xs ${msg.sender?._id === user?._id && !msg.isInternal ? "text-purple-200" : "text-gray-400"}`}
                                            >
