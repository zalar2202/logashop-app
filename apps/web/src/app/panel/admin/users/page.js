...
function UsersPageContent() {
    ...
    // Only depend on searchParams - syncing searchTerm from URL happens when URL changes (debounce, nav).
    // Including searchTerm would cause this effect to overwrite user input on every keystroke.
    useEffect(() => {
        ...
        // Sync local search term state from URL (when URL changes via debounce or browser nav)
    ...
    return (
        <ContentWrapper>
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
                ...
                <Button
                    onClick={() => router.push("/panel/admin/users/create")}
                    icon={<Plus size={18} />}
                                        ...
                                        <TableCell align="right">
                                            <div className="flex items-center justify-end gap-1">
                                                <TableActions
                                                    onView={() =>
                                                        router.push(
                                                            `/panel/admin/users/${user._id}`
                                                        )
                                                    }
                                                    onEdit={() =>
                                                        router.push(
                                                            `/panel/admin/users/${user._id}/edit`
                                                        )
                                                    }
